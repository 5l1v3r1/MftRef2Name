...
Global $IsolatedAttributeList, $AttribListNonResident=0,$IsCompressed,$IsSparse, $_COMMON_KERNEL32DLL=DllOpen("kernel32.dll"),$LogicalClusterNumberforthefileMFT, $ClustersPerFileRecordSegment, $MftAttrListString, $SplitMftRecArr[1]
...
Global $AttributesArr[18][4],$RecordHdrArr[16][2],$FNArr[14][1],$IRArr[12][2],$IndxArr[20][2]
...
If StringLen($StartStr)=2 Then $StartStr&="\"
$TargetDrive = StringMid($StartStr,1,2)
$ParentDir = _GenDirArray($StartStr)
...
If StringIsDigit($IndexNumber) Then
	...
	$ResolvedPath = $TargetDrive & "\" & $ResolvedPath
	ConsoleWrite("Path: " & $ResolvedPath & @CRLF)
	Global $DataQ[1],$AttribX[1],$AttribXType[1],$AttribXCounter[1]
	$NewRecord = _FindFileMFTRecord($IndexNumber)
	_DecodeMFTRecord($NewRecord,2)
	_DumpInfo()
...
If $DirArray[0] = 2 And $DirArray[2] = "" Then
	Global $DataQ[1],$AttribX[1],$AttribXType[1],$AttribXCounter[1]
	$NewRecord = _FindFileMFTRecord(5)
	_DecodeMFTRecord($NewRecord,1)
	ConsoleWrite("Path: " & $TargetDrive & "\" & @CRLF)
	_DumpInfo()
	ConsoleWrite(@CRLF)
	_End($Timerstart)
	Exit
...
For $i = 2 To $DirArray[0]
	...
	If @error Then
		...
		Exit
		Global $DataQ[1],$AttribX[1],$AttribXType[1],$AttribXCounter[1]
		$NewRecord = _FindFileMFTRecord($MftRefArray[$i-1])
		_DecodeMFTRecord($NewRecord,1)
		$LastCheck = _DisplayList($ResolvedPath)
	ElseIf $i=$DirArray[0] Then
		Global $DataQ[1],$AttribX[1],$AttribXType[1],$AttribXCounter[1]
		$NewRecord = _FindFileMFTRecord($MftRefArray[$i])
		_DecodeMFTRecord($NewRecord,1)
		ConsoleWrite("Path: " & $ResolvedPath & "\" & $DirArray[$i] & @CRLF)
		_DumpInfo()
